(function() {
    ...
    audioController.prototype.supportAutoPlay=function(obj){
        ...
        var autoPlayEvents=["play","waiting"];
        function suportTrue(){obj.support&&!supportAuto&&obj.support();supportAuto=true};
        autoPlayTestAudio.events({
            "play":suportTrue,
            "waiting":suportTrue,
            "suspend":function(){obj.nonsupport&&obj.nonsupport();supportAuto=false}
        })
        autoPlayTestAudio.play(obj.url);    
        autoPlayTestAudio.pause(obj.url);
        return supportAuto;   
    ...
    function addAudioEventListener(eventName, backFun) {
        ...
        inListener[eventName] && (removeAudioEventListener.call(this, eventName, inListener[eventName]))
     ...
     * @param  {function} backFun   事件响应执行的函数
     */
    function removeAudioEventListener(eventName, backFun) {
        var audio = this.audio;
        if (doc.removeEventListener) {
            audio.removeEventListener(eventName, backFun);
            return true;
        }
    ...
    function eventsObjectValid(eventsObject) {
        if (eventsObject instanceof Object) {
            for (var key in eventsObject) {
                addAudioEventListener.call(this, key, eventsObject[key]);
            }
        } else {
